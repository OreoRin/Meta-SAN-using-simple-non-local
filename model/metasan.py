...
class _NonLocalBlockND(nn.Module):
    def __init__(self, in_channels, inter_channels=None, dimension=3, sub_sample=True, bn_layer=True):
        ...
        if sub_sample:
            ...
            self.g = nn.Sequential(self.g, nn.AvgPool2d(kernel_size=4,stride=4))
            self.phi = nn.Sequential(self.phi, nn.AvgPool2d(kernel_size=4,stride=4))

            # self.g = nn.Sequential(self.g, torch.nn.UpsamplingBilinear2d(kernel_size=2))
            # self.phi = nn.Sequential(self.phi, torch.nn.UpsamplingBilinear2d(kernel_size=2))
